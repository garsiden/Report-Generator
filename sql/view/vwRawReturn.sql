...
FROM
(
        SELECT [Date], [Value], AssetClassID
...
SELECT  t .Date, LOG (t .CASH / f.CASH) AS CASH, LOG (t .COMM / f.COMM) AS COMM, Log (t .COPR / f.COPR) AS COPR, LOG (t .GLEQ / f.GLEQ) AS GLEQ, 
...
FROM (SELECT [Date], [Value], AssetClassID
FROM tblHistoricData) piv PIVOT (MAX([Value]) FOR AssetClassID IN (CASH, COMM, COPR, GLEQ, HEDG, LOSH, PREQ, UKCB, UKEQ, UKGB, UKHY, WOBO)
) AS chld) t
WHERE f.rn = t .rn
